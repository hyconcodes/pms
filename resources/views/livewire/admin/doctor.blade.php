...
new class extends Component {
    ...
    {
        $this->loadUsers();
        $this->allSpecializations = Specialization::select('id', 'name')->orderBy('name')->get()->toArray();
    }

    public function loadUsers()
    {
        $this->users = User::role('doctor')->latest()->get();
    }
    ...
    public function create()
    {
        ...
        try {
            ...
            $this->reset(['name', 'email', 'password', 'specialization_id']);
    ...
    public function update()
    {
        ...
        try {
            ...
            $this->reset(['name', 'email', 'password', 'userId', 'editMode', 'showModal', 'specialization_id']);
            $this->loadUsers();
    ...
    public function delete($id)
    {
        ...
        try {
            ...
            $this->loadUsers();
            session()->flash('message', '🗑️ Doctor deleted successfully!');
        } catch (\Exception $e) {
    ...
    {
        $this->reset(['name', 'email', 'password', 'editMode', 'userId', 'showModal', 'specialization_id']);
    }
}; ?>
...
<main class="min-h-screen bg-zinc-50 dark:bg-zinc-900 py-8 px-3 sm:px-6 lg:px-8 rounded">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8">
            <div>
                <h1 class="text-3xl font-bold text-green-800 dark:text-green-100">Doctor Management</h1>
                <p class="text-sm text-zinc-600 dark:text-zinc-400 mt-1">
                    Manage doctors, their accounts & specializations easily.
                </p>
            </div>
            @can('create.staff')
                        ...
                        @forelse ($users as $user)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700/40 transition">
                                ...
                                <td class="px-6 py-4 text-right text-sm space-x-2">
                                    ...
                                    @can('delete.staff')
                                        <flux:button
                                            x-on:click.prevent="confirm('Are you sure you want to delete this doctor?') && $wire.delete({{ $user->id }})"
            ...
            <div class="w-full max-w-xl bg-white dark:bg-zinc-800 rounded-xl shadow-xl overflow-hidden relative p-6">
                ...
                <form wire:submit.prevent="{{ $editMode ? 'update' : 'create' }}" class="space-y-5">
                    <div class="space-y-5">
                        ...
                        <div>
                            <label for="specialization" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Specialization</label>
                            <flux:select wire:model="specialization_id" id="specialization" 
                                class="mt-1 block w-full rounded-md border-zinc-300 shadow-sm focus:border-green-500 focus:ring-green-500 dark:border-zinc-600 dark:bg-zinc-700 dark:text-zinc-300">
                                <option value="">Select a specialization</option>
                                @foreach($allSpecializations as $specialization)
                                    <option value="{{ $specialization['id'] }}">{{ $specialization['name'] }}</option>
                                @endforeach
                            </flux:select>
                            @error('specialization_id') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>
