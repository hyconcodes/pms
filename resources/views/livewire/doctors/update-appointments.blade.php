        ...
        @if ($appointment)
            <div class="bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-2xl shadow-xl border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="px-6 py-5 border-b border-slate-200/60 dark:border-slate-700/60 flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100">Update Appointment</h1>
                        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Reference: {{ $appointment->apid }}</p>
                    </div>
                ...
                <form wire:submit.prevent="saveAppointment" class="p-6 grid gap-6">
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-slate-800 dark:text-slate-100 min-h-[2.5rem] bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">{{ $appointment->reason_for_visit ?: 'No reason provided' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Specialty</label>
                            <p class="text-slate-800 dark:text-slate-100 min-h-[2.5rem] bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">{{ $appointment->specialty ?: '—' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Appointment Date</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Appointment Time</label>
                            <p class="text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">{{ $appointment->appointment_time }}</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        @else
                            <p class="text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3 capitalize">{{ $appointment->status }}</p>
                        @endif
                    </div>

                    <!-- Diagnosis -->
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Diagnosis</label>
                        @if ($editMode)
                        ...
                        @else
                            <p class="text-slate-800 dark:text-slate-100 min-h-[4.5rem] bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">{{ $appointment->diagnosis ?: '—' }}</p>
                        @endif
                    </div>

                    <!-- Symptoms -->
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Symptoms</label>
                        @if ($editMode)
                        ...
                        @else
                            <p class="text-slate-800 dark:text-slate-100 min-h-[4.5rem] bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">{{ $appointment->symptoms ?: '—' }}</p>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        @else
                            <p class="text-slate-800 dark:text-slate-100 min-h-[4.5rem] bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">{{ $appointment->notes ?: '—' }}</p>
                        @endif
                    </div>
                    ...
                    <div class="grid md:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Blood Pressure</label>
                            @if ($editMode)
                            ...
                            @else
                                <p class="text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">{{ $appointment->blood_pressure ?: '—' }}</p>
                            @endif
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Temperature (°C)</label>
                            @if ($editMode)
                            ...
                            @else
                                <p class="text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">{{ $appointment->temperature ?: '—' }}</p>
                            @endif
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Heart Rate (bpm)</label>
                            @if ($editMode)
                            ...
                            @else
                                <p class="text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">{{ $appointment->heart_rate ?: '—' }}</p>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="grid md:grid-cols-3 gap-4">
                        <div>
                            ...
                            @else
                                <p class="text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">{{ $appointment->weight ?: '—' }}</p>
                            @endif
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Height (cm)</label>
                            @if ($editMode)
                            ...
                            @else
                                <p class="text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">{{ $appointment->height ?: '—' }}</p>
                            @endif
                        </div>
                        <div>
                            ...
                            @else
                                <p class="text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">{{ $appointment->allergies ?: '—' }}</p>
                            @endif
                        </div>
                    </div>
                    ...
                    <!-- Lab Results -->
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Lab Results</label>
                        @if ($editMode)
                        ...
                        @else
                            <p class="text-slate-800 dark:text-slate-100 min-h-[4.5rem] bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">{{ $appointment->lab_results ?: '—' }}</p>
                        @endif
                    </div>
